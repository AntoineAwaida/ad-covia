...
class Image:
    def __init__(self, opencv_image, name: str):
        self.opencv_data = opencv_image
        self.name = name
    ...
    def crop(self, vert_frac: float = 0.0, horiz_frac: float = 0.0) -> "Image":
        start_vert, start_horiz = self._fraction_to_pixels(
            (vert_frac / 2, horiz_frac / 2)
        )
        end_vert, end_horiz = self._fraction_to_pixels(
            (1 - vert_frac / 2, 1 - horiz_frac / 2)
        )

        image = self.opencv_data[start_vert:end_vert, start_horiz:end_horiz]
        return Image(
            image,
            f"{self.name} cropped to ({image.shape[1]}x{image.shape[0]})",
        )
    ...
    def show(self) -> None:
        cv2.imshow(self.name, self.opencv_data)
        cv2.waitKey()

    def __getitem__(self, item: int):
        return Image(self.opencv_data[item], self.name)

    def __str__(self) -> str:
        return str(self.opencv_data)
